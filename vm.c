...
recheck:
  ...
  {
    if (proc->ramCtrlr[i].state == INUSE && proc->ramCtrlr[i].loadOrder <= loadOrder)
    {
      pageIndex = i;
...
int getLRU()
{
  ...
  for (i = 0; i < MAX_PYSC_PAGES; i++)
  {
    if (proc->ramCtrlr[i].state == INUSE && proc->ramCtrlr[i].accessCount <= minAccess)
    {
...
int getPageOutIndex()
{
  ...
  return getLRU();
#endif
  panic("Unrecognized page replacement algorithm");
}
...
void addToRamCtrlr(pde_t *pgdir, uint userPageVAddr) {
  ...
  proc->ramCtrlr[freeLocation].state = INUSE;
...
void removeFromRamCtrlr(uint userPageVAddr, pde_t *pgdir){
  ...
  int i;
  for (i = 0; i < MAX_PYSC_PAGES; i++) {
    if (proc->ramCtrlr[i].state == INUSE && proc->ramCtrlr[i].userPageVAddr == userPageVAddr && proc->ramCtrlr[i].pgdir == pgdir)
    {
...
void removeFromFileCtrlr(uint userPageVAddr, pde_t *pgdir){
  ...
  for (i = 0; i < MAX_TOTAL_PAGES-MAX_PYSC_PAGES; i++) {
    if (proc->fileCtrlr[i].state == INUSE && proc->fileCtrlr[i].userPageVAddr == userPageVAddr && proc->fileCtrlr[i].pgdir == pgdir)
    {
